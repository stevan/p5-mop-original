...
These methods provide access to the superclass list, as
well as the compiled method resolution order of a class.

=over 4

=item C<get_superclasses>

This returns an array reference which contains the superclasses
for this given class.

=item C<add_superclasses( \@superclasses )>

This accepts an array reference of class objects which will be
pushed onto the end of the superclass list.
