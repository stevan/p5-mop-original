...
The proposed MOP is constructed of four sub protocols,
...
state of a class. Just a class can be viewed as the
...
The concept of an instance is a value which knows from
what class it was created and contains a set of slots
which correspond to the attributes in the class (and
any superclasses).

The instance sub-protocol would have methods to
find the associated class, get/set the values in the
slots or list the set of slots.
...
a C<$name> attribute, a C<$superclasses> attribute, an
...
is where the bootstrap process comes into play. We simply
